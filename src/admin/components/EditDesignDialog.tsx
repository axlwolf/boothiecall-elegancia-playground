...
interface EditDesignDialogProps {
  ...
  design: Design | null;
  onUpdate: (id: string, data: Partial<Omit<Design, 'id'>>) => void;
}

export const EditDesignDialog: React.FC<EditDesignDialogProps> = ({ open, onOpenChange, design, onUpdate }) => {
  ...
  useEffect(() => {
    if (design) {
      ...
      setFrameMappings(design.frameMappings);
  ...
  const handleSubmit = () => {
    if (design) {
      onUpdate(design.id, { name, layoutType, frameMappings, isActive });
      onOpenChange(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-gray-800 border-gray-700 text-white">
        <DialogHeader>
          <DialogTitle className="text-gold-500">Edit Design</DialogTitle>
          <DialogDescription>Update the design details.</DialogDescription>
        </DialogHeader>
