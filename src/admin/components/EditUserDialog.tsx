...
interface EditUserDialogProps {
  ...
  user: User | null;
  onUpdate: (id: string, data: Partial<Omit<User, 'id'>>) => void;
}

export const EditUserDialog: React.FC<EditUserDialogProps> = ({ open, onOpenChange, user, onUpdate }) => {
  ...
  const handleSubmit = () => {
    if (user) {
      onUpdate(user.id, { email, role, tenantId });
      onOpenChange(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-gray-800 border-gray-700 text-white">
        <DialogHeader>
          <DialogTitle className="text-gold-500">Edit User</DialogTitle>
